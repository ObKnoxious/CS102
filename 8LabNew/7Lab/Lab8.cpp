...
int main(){
	...
	bool g = true;
	while(g){
		int o2=ScoreHand(deck,players[0]);
		//Final output
		cout << "Dealer has cards: ";
		string of = players[0][0].substr(0, players[0][0].size()-1) + " " + players[0][1].substr(0, players[0][0].size()-1);
		...
		o2=ScoreHand(deck,players[1]);
		cout << "Player  has cards: ";
		of = players[1][0].substr(0, players[0][0].size()-1) + " " + players[1][1].substr(0, players[0][0].size()-1);
		stringstream oy2;
		oy2 << o2;
		ox = "(" + oy2.str() + ")";
		cout << left << setw(20) << of << setw(4) << right << ox << "\n";
		...
		cin >>  pin;
		if(pin == "stand"){
			while(ScoreHand(deck,players[0]) < 18){
					players[0].push_back(DealCard(deck));
					int ob = ScoreHand(deck,players[0]);
					//cout << ob << "\n";
					stringstream oy3; oy3 << ob;
					ox = "(" +oy3.str() + ")";
					cout << "Dealer hit, dealer got " << left << setw(20) << players[0].back().substr(0, players[0][0].size()-1) << setw(4) << right << ox << "\n";
				}
			g = false;
		 } else if(pin == "hit"){
			players[1].push_back(DealCard(deck));
			int ps = ScoreHand(deck,players[1]);
			if(ScoreHand(deck,players[0]) < 18){
				 players[0].push_back(DealCard(deck));
			}
			string dc = "Dealer has cards : ";
			string od;
			for(int i=0; i<players[0].size(); i++){
				od += players[0][i].substr(0, players[0][0].size()-1) + " ";
			}
			int ob = ScoreHand(deck,players[0]);
			//cout << ob << "\n";
			stringstream oy3; oy3 << ob;
			...
			string pc = "Player has cards : ";
			string od1;
			for(int i=0; i<players[0].size(); i++){
				od1 += players[0][i].substr(0, players[0][0].size()-1) + " ";
			}
			int ob1 = ScoreHand(deck,players[0]);
			//cout << ob << "\n";
			stringstream oy4; oy4 << ob1;
			ox = "(" +oy4.str() + ")";
			cout << pc + od1  + ox << "\n";
			if(ScoreHand(deck,players[0]) > 21){
				cout << "Dealer bust, players wins! \n";
				return 0;
			} else if(ScoreHand(deck,players[1]) > 21){
				cout << "Player bust, Dealer wins! \n";
				return 0;
			}
		 }
		// This is all incredibly sloppy and bad but I"ve been working for 5 hours straight and just want this done
